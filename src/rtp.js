...
(function() {
  ...
  RTPParser = (function() {
    ...
    RTPParser.prototype.feedUnorderedPacket = function(tag, packet) {
      var buffers, diff, discardedSequenceNumber, firstPacket, latestSequenceNumber, packetBuffer, _results;
      ...
      if (packetBuffer.nextSequenceNumber === packet.rtpHeader.sequenceNumber) {
        ...
        if (buffers.length >= 2) {
          ...
          while (buffers.length >= 2) {
            ...
            if (packetBuffer.nextSequenceNumber > MAX_SEQUENCE_NUMBER) {
              _results.push(packetBuffer.nextSequenceNumber = 0);
            } else {
    ...
    RTPParser.prototype.onOrderedPacket = function(tag, packet) {
      var clientId, match, nalUnit, _i, _len, _ref, _results;
      if ((match = /^h264:(.*)$/.exec(tag)) != null) {
        ...
        if (packet.h264.fu_a != null) {
          ...
          _ref = packet.h264.stap_a.nalUnits;
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            nalUnit = _ref[_i];
            _results.push(this.onH264NALUnit(clientId, nalUnit, packet, packet.rtpHeader.timestamp));
          }
          return _results;
        } else {
  ...
  api = {
    ...
    readRTCPSenderReport: function(bits) {
      var i, info, readBytes, reportBlock, startBytePos, _i, _ref;
      ...
      info.reportBlocks = [];
      for (i = _i = 0, _ref = info.reportCount; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        reportBlock = {};
    ...
    readRTCPSourceDescription: function(bits) {
      var bytesPastBoundary, chunk, i, info, moreSDESItems, nullOctet, readBytes, sdesItem, startBytePos, _i, _ref;
      startBytePos = bits.current_position().byte;
      info = {};
      info.version = bits.read_bits(2);
      info.padding = bits.read_bit();
      ...
      info.totalBytes = (info.wordsMinusOne + 1) * 4;
      info.chunks = [];
      for (i = _i = 0, _ref = info.sourceCount; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        chunk = {};
    ...
    readRTPFixedHeader: function(bits) {
      ...
      info.ssrc = bits.read_bits(32);
      info.csrc = [];
      for (i = _i = 0, _ref = info.csrcCount; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
    ...
    readH264Payload: function(bits) {
      ...
      if ((1 <= (_ref = info.nal_unit_type) && _ref <= 23)) {
        bits.push_back_byte();
        info.nal_unit = bits.remaining_buffer();
      } else if ((24 <= (_ref1 = info.nal_unit_type) && _ref1 <= 29)) {
    ...
    readAACPayload: function(bits, params) {
      var accessUnit, auHeader, auHeaders, i, info, _i, _j, _len, _ref;
      info = {};
      info.auHeadersLengthBits = bits.read_bits(16);
      info.numAUHeaders = info.auHeadersLengthBits / 16;
      auHeaders = [];
      for (i = _i = 0, _ref = info.numAUHeaders; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        params.index = i;
        auHeaders.push(api.readAACAUHeader(bits, params));
      }
      info.auHeaders = auHeaders;
      info.accessUnits = [];
      for (_j = 0, _len = auHeaders.length; _j < _len; _j++) {
        auHeader = auHeaders[_j];
    ...
    createAudioHeader: function(opts) {
      var accessUnit, header, numBits, _i, _len, _ref;
    ...
    groupAudioFrames: function(adtsFrames) {
      var adtsFrame, currentGroup, groups, i, packetSize, _i, _len;
    ...
    createRTPHeader: function(opts) {
      var seqNum, ssrc, ts, _ref;
      seqNum = opts.sequenceNumber;
      ts = opts.timestamp;
      ssrc = (_ref = opts.ssrc) != null ? _ref : 0;
    ...
    createGoodbye: function(opts) {
      var data, length, reason, ssrc, ssrcs, _i, _len;
      ...
      reason = __slice.call(new Buffer("End of stream", 'utf8'));
      ...
      for (_i = 0, _len = ssrcs.length; _i < _len; _i++) {
        ssrc = ssrcs[_i];
