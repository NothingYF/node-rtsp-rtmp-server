...
(function() {
  ...
  exports.UDPClient = UDPClient = (function() {
    ...
    UDPClient.prototype.write = function(buf, callback) {
      ...
      if (this.isInBlockMode) {
        this.pendingPackets.push([this.write].concat(__slice.call(arguments)));
        return;
      }
    ...
    UDPClient.prototype._writeReliable = function(buf, packetId, callback) {
      if (this.isInBlockMode) {
        this.pendingPackets.push([this._writeReliable].concat(__slice.call(arguments)));
        return;
      }
    ...
    UDPClient.prototype.writeReliableBlocked = function(buf, callback) {
      var packetId;
      if (this.isInBlockMode) {
        this.pendingPackets.push([this.writeReliableBlocked].concat(__slice.call(arguments)));
        return;
      }
  ...
  exports.UDPServer = UDPServer = (function(_super) {
    ...
    UDPServer.prototype.onReceiveMessage = function(msg, rinfo) {
      var e, endFragmentNumber, fragmentNumber, i, isMissing, packetId, packetType, receivedBuf, targetBuf, _i;
      packetType = msg[0];
      packetId = msg[1];
      ...
      if (endFragmentNumber > 0) {
        ...
        isMissing = false;
        for (i = _i = 0; 0 <= endFragmentNumber ? _i <= endFragmentNumber : _i >= endFragmentNumber; i = 0 <= endFragmentNumber ? ++_i : --_i) {
    ...
    UDPServer.prototype.deleteOldBufferedPackets = function() {
      var isDoneSomething, nextPacketId, oldEnoughTime, oldestUnprocessedPacketId, packetId, timeDiff, _i, _ref;
      ...
      oldEnoughTime = Date.now() - OLD_UDP_PACKET_TIME_THRESHOLD;
      for (packetId = _i = oldestUnprocessedPacketId, _ref = this.latestPacketId; oldestUnprocessedPacketId <= _ref ? _i < _ref : _i > _ref; packetId = oldestUnprocessedPacketId <= _ref ? ++_i : --_i) {
    ...
    UDPServer.prototype.onReceivePacket = function(packet) {
      var anticipatingPacketId, nextPacketId, _ref;
      ...
      if (packet.packetId === anticipatingPacketId) {
        ...
        if ((this.processedPacketId - RECEIVE_PACKET_ID_WINDOW <= (_ref = packet.packetId) && _ref <= this.processedPacketId)) {
