...
(function() {
  var Bits, CRLF_CRLF, DAY_NAMES, DEBUG_DISABLE_UDP_TRANSPORT, DEBUG_HTTP_TUNNEL, DEBUG_OUTGOING_PACKET_DATA, DEBUG_OUTGOING_RTCP, DEBUG_RTSP, DEBUG_RTSP_ONLY_HEADERS, DEFAULT_SERVER_NAME, ENABLE_START_PLAYING_FROM_KEYFRAME, MONTH_NAMES, RTSPClient, RTSPServer, SINGLE_NAL_UNIT_MAX_SIZE, TAG, TIMESTAMP_ROUNDOFF, aac, api, avstreams, config, crypto, dgram, generateNewSessionID, generateRandom32, h264, http, logger, net, os, pad, resetStreamParams, rtp, sdp, url, zeropad,
    __slice = [].slice;
  ...
  generateRandom32 = function() {
    var md5sum, str;
    str = ("" + (new Date().getTime()) + process.pid + (os.hostname())) + ("" + (process.getuid()) + (process.getgid())) + (1 + Math.random() * 1000000000);
  ...
  RTSPServer = (function() {
    function RTSPServer(opts) {
      ...
      this.numClients = 0;
      this.eventListeners = {};
      this.serverName = (_ref = opts != null ? opts.serverName : void 0) != null ? _ref : DEFAULT_SERVER_NAME;
      this.port = (_ref1 = opts != null ? opts.port : void 0) != null ? _ref1 : 8080;
      this.clients = {};
      this.httpSessions = {};
    ...
    RTSPServer.prototype.getVideoSendTimeForUploadingRTPTimestamp = function(stream, rtpTimestamp) {
      var rtpDiff, timeDiff, videoTimestampInfo, _ref;
      videoTimestampInfo = (_ref = stream.rtspUploadingClient) != null ? _ref.uploadingTimestampInfo.video : void 0;
    ...
    RTSPServer.prototype.getAudioSendTimeForUploadingRTPTimestamp = function(stream, rtpTimestamp) {
      var audioTimestampInfo, rtpDiff, timeDiff, _ref;
      audioTimestampInfo = (_ref = stream.rtspUploadingClient) != null ? _ref.uploadingTimestampInfo.audio : void 0;
    ...
    RTSPServer.prototype.sendVideoData = function(stream, nalUnits, pts, dts) {
      var i, isLastPacket, isPPSSent, isSPSSent, nalUnit, nalUnitType, _i, _len;
      isSPSSent = false;
      isPPSSent = false;
      for (i = _i = 0, _len = nalUnits.length; _i < _len; i = ++_i) {
        nalUnit = nalUnits[i];
    ...
    RTSPServer.prototype.sendAudioData = function(stream, accessUnits, pts, dts) {
      var accessUnitLength, audioHeader, client, clientID, concatRawDataBlock, frameGroups, group, i, processedFrames, rtpBuffer, rtpData, rtpTimePerFrame, timestamp, ts, _i, _len, _ref;
      ...
      processedFrames = 0;
      for (i = _i = 0, _len = frameGroups.length; _i < _len; i = ++_i) {
        group = frameGroups[i];
    ...
    RTSPServer.prototype.sendEOS = function(stream) {
      var buf, client, clientID, _ref, _results;
      _ref = stream.rtspClients;
      _results = [];
      for (clientID in _ref) {
        client = _ref[clientID];
        ...
        if (client.useTCPForAudio) {
          if (client.useHTTP) {
            if (client.httpClientType === 'GET') {
              _results.push(this.sendDataByTCP(client.socket, client.audioTCPControlChannel, buf));
            } else {
          ...
          if (client.clientAudioRTCPPort != null) {
            _results.push(this.audioRTCPSocket.send(buf, 0, buf.length, client.clientAudioRTCPPort, client.ip, function(err, bytes) {
              if (err) {
    ...
    RTSPServer.prototype.start = function(opts, callback) {
      var serverPort, udpAudioControlServer, udpAudioDataServer, udpVideoControlServer, udpVideoDataServer, _ref;
      ...
      this.server = net.createServer((function(_this) {
        return function(c) {
          ...
          return generateNewSessionID(function(err, sessionID) {
            ...
            c.responseCount = 0;
            c.on('close', function() {
              var addr, e, _client, _ref1;
              logger.info("[" + TAG + "] client " + id_str + " is closed");
              logger.debug("[client:" + _this.id + "] teardown: session=" + _this.sessionID);
              try {
                c.end();
              } catch (_error) {
                e = _error;
              ...
              _ref1 = _this.rtspUploadingClients;
              for (addr in _ref1) {
                _client = _ref1[addr];
                if (_client === client) {
    ...
    RTSPServer.prototype.stop = function(callback) {
      var _ref;
      return (_ref = this.server) != null ? _ref.close(callback) : void 0;
    };
    ...
    RTSPServer.getStreamIdFromUri = function(uri) {
    ...
    RTSPServer.prototype.handlePOSTData = function(client, data, callback) {
      ...
      processRemainingBuffer = (function(_this) {
        return function() {
          if ((client.postBase64Buf != null) || (client.postBuf != null)) {
            _this.handlePOSTData(client, '', callback);
          } else {
            if (typeof callback === "function") {
              callback(null);
            }
      ...
      if (postData[0] === api.INTERLEAVED_SIGN) {
    ...
    RTSPServer.prototype.handleOnData = function(c, data) {
      ...
      if (client.isSendingPOST) {
        this.handlePOSTData(client, data.toString('utf8'));
        return;
      }
      if (c.buf != null) {
      ...
      return this.respond(c, req, (function(_this) {
        return function(err, output, resultOpts) {
          var delimPos, headerBytes, i, out, _i, _len;
          if (err) {
            logger.error("[respond] Error: " + err);
            return;
          }
    ...
    RTSPServer.prototype.sendVideoPacketWithFragment = function(stream, nalUnit, timestamp, marker) {
      var client, clientID, fragmentNumber, isKeyFrame, nalUnitLen, nalUnitType, nal_ref_idc, rtpBuffer, rtpData, thisNalUnit, thisNalUnitLen, ts, _ref, _ref1;
      if (marker == null) {
        marker = true;
      }
      ...
      while (nalUnit.length > SINGLE_NAL_UNIT_MAX_SIZE) {
        ...
        for (clientID in _ref) {
          ...
          if (client.isPlaying) {
            ...
            logger.tag('rtsp:out', ("[rtsp:stream:" + stream.id + "] send video to " + client.id + ": fragment n=" + fragmentNumber + " timestamp=" + ts + " bytes=" + rtpBuffer.length + " marker=false") + (isKeyFrame ? " isKeyFrame=" + isKeyFrame : ""));
      ...
      _ref1 = stream.rtspClients;
      for (clientID in _ref1) {
        client = _ref1[clientID];
        if (client.isWaitingForKeyFrame && isKeyFrame) {
          process.stdout.write("KeyFrame");
          client.isPlaying = true;
          client.isWaitingForKeyFrame = false;
        ...
        if (client.isPlaying) {
          ...
          logger.tag('rtsp:out', ("[rtsp:stream:" + stream.id + "] send video to " + client.id + ": fragment-last n=" + (fragmentNumber + 1) + " timestamp=" + ts + " bytes=" + rtpBuffer.length + " marker=" + marker) + (isKeyFrame ? " isKeyFrame=" + isKeyFrame : ""));
          if (client.useTCPForVideo) {
            if (client.useHTTP) {
              if (client.httpClientType === 'GET') {
                this.sendDataByTCP(client.socket, client.videoTCPDataChannel, rtpBuffer);
    ...
    RTSPServer.prototype.sendVideoPacketAsSingleNALUnit = function(stream, nalUnit, timestamp, marker) {
      var client, clientID, isKeyFrame, nalUnitLen, nalUnitType, rtpBuffer, rtpHeader, ts, _ref;
      if (marker == null) {
        marker = true;
      }
      if (++stream.videoSequenceNumber > 65535) {
    ...
    RTSPServer.prototype.respondWithNotFound = function(protocol, callback) {
      var res;
      if (protocol == null) {
        protocol = 'RTSP';
      }
    ...
    RTSPServer.prototype.respondPost = function(socket, req, callback) {
      ...
      if (config.enableRTMPT && /^\/(?:fcs|open|idle|send|close)\//.test(pathname)) {
        ...
        this.rtmpServer.handleRTMPTRequest(req, (function(_this) {
          return function(err, output, resultOpts) {
            if (err) {
              logger.error("[rtmpt] Error: " + err);
              return _this.respondWithNotFound('HTTP', function(err, response) {
                return callback(response, resultOpts);
              });
            } else {
              return callback(err, output, resultOpts);
            }
          };
        })(this));
        return;
      ...
      if (this.httpSessions[req.headers['x-sessioncookie']] == null) {
        this.respondWithNotFound('HTTP', callback);
        return;
      }
      ...
      if (req.body != null) {
        return this.handlePOSTData(client, req.body);
      }
    };

    RTSPServer.prototype.respondGet = function(socket, req, callback) {
      var client, match, pathname;
      client = this.clients[socket.clientID];
      pathname = url.parse(req.uri).pathname;
      if (pathname === '/crossdomain.xml') {
        this.httpHandler.respondCrossDomainXML(req, function(err, output) {
          var _ref;
          return callback(err, output, {
            close: ((_ref = req.headers.connection) != null ? _ref.toLowerCase() : void 0) !== 'keep-alive'
          });
        });
      } else if ((match = /^\/live\/(.*)$/.exec(req.uri)) != null) {
        return this.consumePathname(req.uri, (function(_this) {
          return function(err) {
            var postClient, res;
            if (err) {
              logger.warn("Failed to consume pathname: " + err);
              _this.respondWithNotFound('HTTP', callback);
              return;
            }
            client.sessionCookie = req.headers['x-sessioncookie'];
            client.useHTTP = true;
    ...
    RTSPServer.prototype.respondDescribe = function(socket, req, callback) {
      ...
      return this.consumePathname(req.uri, (function(_this) {
        return function(err) {
          var ascInfo, body, dateHeader, e, res, sdpData, stream;
          if (err) {
            _this.respondWithNotFound('RTSP', callback);
            return;
          }
          socket.isAuthenticated = true;
          client.bandwidth = req.headers.bandwidth;
          stream = _this.getStreamByUri(req.uri);
          if (stream == null) {
            _this.respondWithNotFound('RTSP', callback);
            return;
          }
          ...
          if (stream.isAudioStarted) {
            ...
            if ((ascInfo != null ? ascInfo.explicitHierarchicalSBR : void 0) && config.rtspDisableHierarchicalSBR) {
              logger.debug("[rtsp] converting hierarchical signaling of SBR" + (" (AudioSpecificConfig=0x" + (stream.audioSpecificConfig.toString('hex')) + ")") + " to backward compatible signaling");
    ...
    RTSPServer.prototype.respondSetup = function(socket, req, callback) {
      var ch1, ch2, client, controlPort, control_ch, dataPort, data_ch, dateHeader, match, media, mediaType, mode, res, sdpInfo, serverPort, setupStreamId, ssrc, stream, streamId, target, track, transportHeader, useTCPTransport, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
      client = this.clients[socket.clientID];
      if (!socket.isAuthenticated) {
        this.respondWithNotFound('RTSP', callback);
        return;
      }
      serverPort = null;
      track = null;
      ...
      if (mode === 'RECORD') {
        ...
        if ((match = /\/([^/]+)$/.exec(req.uri)) != null) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            media = _ref[_i];
            if (((_ref1 = media.attributes) != null ? _ref1.control : void 0) === setupStreamId) {
        ...
        if ((match = /;interleaved=(\d)-(\d)/.exec(req.headers.transport)) != null) {
          ...
          if (mediaType === 'video') {
            _ref2 = [config.rtspVideoDataUDPListenPort, config.rtspVideoControlUDPListenPort], dataPort = _ref2[0], controlPort = _ref2[1];
            ...
            _ref3 = [config.rtspAudioDataUDPListenPort, config.rtspAudioControlUDPListenPort], dataPort = _ref3[0], controlPort = _ref3[1];
            if ((match = /;client_port=(\d+)-(\d+)/.exec(req.headers.transport)) != null) {
        ...
        if (/\bTCP\b/.test(req.headers.transport)) {
          ...
          if ((match = /;interleaved=(\d+)-(\d+)/.exec(req.headers.transport)) != null) {
            ...
            if (ch1 % 2 === 0) {
              _ref4 = [ch1, ch2], data_ch = _ref4[0], control_ch = _ref4[1];
            } else {
              _ref5 = [ch2, ch1], data_ch = _ref5[0], control_ch = _ref5[1];
            }
          } else {
            if (track === 'audio') {
              data_ch = 0;
    ...
    RTSPServer.prototype.respondPlay = function(socket, req, callback) {
      var baseUrl, client, preventFromPlaying, res, rtpInfos, stream;
      client = this.clients[socket.clientID];
      if (!socket.isAuthenticated) {
        this.respondWithNotFound('RTSP', callback);
        return;
      ...
      stream = this.getStreamByUri(req.uri);
      if (stream == null) {
        this.respondWithNotFound('RTSP', callback);
        return;
      }
    ...
    RTSPServer.prototype.respondAnnounce = function(socket, req, callback) {
      var ascInfo, audioObjectType, audioSpecificConfig, client, media, nalUnit, nalUnitType, nalUnits, packetizationMode, res, sdpInfo, stream, streamId, _i, _j, _len, _len1, _ref, _ref1, _ref2;
      client = this.clients[socket.clientID];
      streamId = RTSPServer.getStreamIdFromUri(req.uri);
      stream = avstreams.get(streamId);
      if (stream != null) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (media.media === 'video') {
          sdpInfo.video = media;
          if (((_ref1 = media.fmtpParams) != null ? _ref1['packetization-mode'] : void 0) != null) {
          ...
          if (((_ref2 = media.fmtpParams) != null ? _ref2['sprop-parameter-sets'] : void 0) != null) {
            nalUnits = h264.parseSpropParameterSets(media.fmtpParams['sprop-parameter-sets']);
            for (_j = 0, _len1 = nalUnits.length; _j < _len1; _j++) {
              nalUnit = nalUnits[_j];
              nalUnitType = nalUnit[0] & 0x1f;
              switch (nalUnitType) {
    ...
    RTSPServer.prototype.respond = function(socket, req, callback) {
      if (req.method === 'OPTIONS') {
      ...
      } else if ((req.method === 'POST') && (req.protocol.indexOf('HTTP') !== -1)) {
        return this.respondPost(socket, req, callback);
      } else if ((req.method === 'GET') && (req.protocol.indexOf('HTTP') !== -1)) {
        ...
        return this.respondWithNotFound('RTSP', callback);
      }
    };
    ...
    RTSPServer.prototype.onUploadVideoControl = function(stream, msg, rinfo) {
      ...
      for (_i = 0, _len = packets.length; _i < _len; _i++) {
        ...
        if (packet.rtcpSenderReport != null) {
          ...
          _results.push(stream.rtspUploadingClient.uploadingTimestampInfo.video.time = packet.rtcpSenderReport.ntpTimestampInMs);
        } else {
          _results.push(void 0);
        }
      }
    ...
    RTSPServer.prototype.onUploadAudioControl = function(stream, msg, rinfo) {
      ...
      for (_i = 0, _len = packets.length; _i < _len; _i++) {
        ...
        if (packet.rtcpSenderReport != null) {
          ...
          _results.push(stream.rtspUploadingClient.uploadingTimestampInfo.audio.time = packet.rtcpSenderReport.ntpTimestampInMs);
        } else {
          _results.push(void 0);
        }
      }
  ...
  RTSPClient = (function() {
    ...
    RTSPClient.prototype.toString = function() {
      ...
      if (this.socket.remoteAddress == null) {
        ...
        if ((_ref = this.clientType) === 'http-get' || _ref === 'tcp' || _ref === 'udp') {
  ...
  api = {
    ...
    getLocalIP: function() {
      var addr, getPriority, ifaceName, ifaceNames, ifacePrecedence, ifaces, _i, _j, _len, _len1, _ref;
      ifacePrecedence = ['wlan', 'eth', 'en'];
      getPriority = function(ifaceName) {
        var i, name, _i, _len;
        for (i = _i = 0, _len = ifacePrecedence.length; _i < _len; i = ++_i) {
      ...
      for (_i = 0, _len = ifaceNames.length; _i < _len; _i++) {
        ifaceName = ifaceNames[_i];
        _ref = ifaces[ifaceName];
    ...
    leaveClient: function(client) {
      var stream, streamName, _ref;
      _ref = avstreams.getAll();
      for (streamName in _ref) {
        stream = _ref[streamName];
