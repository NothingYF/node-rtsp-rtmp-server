...
(function() {
  ...
  api = {
    ...
    read_ISO_639_language_descriptor: function(bits) {
      var ISO_639_language_code, audio_type, i, info, num_loops, _i;
      info = {};
      info.descriptor_tag = bits.read_byte();
      info.descriptor_length = bits.read_byte();
      num_loops = info.descriptor_length / 4;
      info.languages = [];
      for (i = _i = 0; 0 <= num_loops ? _i < num_loops : _i > num_loops; i = 0 <= num_loops ? ++_i : --_i) {
    ...
    read_program_association_section: function(bits) {
      var CRC_32, bit_0, i, info, num_programs, program_number, reserved, _i;
      info = {};
      info.pointer_field = bits.read_byte();
      info.table_id = bits.read_byte();
      ...
      info.programTable = {};
      for (i = _i = 0; 0 <= num_programs ? _i < num_programs : _i > num_programs; i = 0 <= num_programs ? ++_i : --_i) {
    ...
    consumeUnparsedPESPackets: function() {
      var pesInfo, pesPacket, _i, _len;
      for (_i = 0, _len = unparsedPESPackets.length; _i < _len; _i++) {
        pesPacket = unparsedPESPackets[_i];
        pesInfo = this.parsePESPacket(pesPacket.pid, pesPacket.packet, pesPacket.opts);
        if (pesInfo.not_parsed) {
          continue;
        }
    ...
    read_system_header: function(bits) {
      ...
      info.system_video_lock_flag = bits.raed_bit();
      marker_bit = bits.read_bit();
    ...
    read_pes_packet: function(bits, opts) {
      var PES_packet_data_byte, bits_0001, bits_0010, bits_0011, bits_01, bits_10, info, marker_bit, packet_start_code_prefix, padding_byte, remaining_header_len, remaining_pes_len, reserved, _ref, _ref1;
      info = {};
      ...
      if ((_ref = info.stream_id) !== PES_STREAM_ID_PROGRAM_STREAM_MAP && _ref !== PES_STREAM_ID_PADDING_STREAM && _ref !== PES_STREAM_ID_PRIVATE_STREAM_2 && _ref !== PES_STREAM_ID_ECM && _ref !== PES_STREAM_ID_EMM && _ref !== PES_STREAM_ID_PROGRAM_STREAM_DIRECTORY && _ref !== PES_STREAM_ID_DSMCC_STREAM && _ref !== PES_STREAM_ID_ITU_T_REC_H_222_1_TYPE_E_STREAM) {
        ...
        if (info.DSM_trick_mode_flag === 1) {
          ...
          } else if (trick_mode_control === TRICK_MODE_CONTROL_SLOW_MOTION) {
            info.rep_cntrl = bits.read_bits(5);
          } else if (trick_mode_control === TRICK_MODE_CONTROL_FREEZE_FRAME) {
            info.field_id = bits.read_bits(2);
            reserved = bits.read_bits(3);
          } else if (trick_mode_control === TRICK_MODE_CONTROL_FAST_REVERSE) {
            info.field_id = bits.read_bits(2);
            info.intra_slice_refresh = bits.read_bit();
            info.frequency_truncation = bits.read_bits(2);
          } else if (trick_mode_control === TRICK_MODE_CONTROL_SLOW_REVERSE) {
            info.rep_cntrl = bits.read_bits(5);
          } else {
            reserved = bits.read_bits(5);
          }
      ...
      } else if ((_ref1 = info.stream_id) === PES_STREAM_ID_PROGRAM_STREAM_MAP || _ref1 === PES_STREAM_ID_PRIVATE_STREAM_2 || _ref1 === PES_STREAM_ID_ECM || _ref1 === PES_STREAM_ID_EMM || _ref1 === PES_STREAM_ID_PROGRAM_STREAM_DIRECTORY || _ref1 === PES_STREAM_ID_DSMCC_STREAM || _ref1 === PES_STREAM_ID_ITU_T_REC_H_222_1_TYPE_E_STREAM) {
    ...
    read_transport_packet: function(bits) {
      var info, remaining_bytes, sync_byte, transport_priority, _ref, _ref1;
      info = {};
