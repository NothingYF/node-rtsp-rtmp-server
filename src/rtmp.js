...
(function() {
  var AVC_PACKET_TYPE_END_OF_SEQUENCE, AVC_PACKET_TYPE_NALU, AVC_PACKET_TYPE_SEQUENCE_HEADER, DEBUG_INCOMING_RTMP_PACKETS, DEBUG_OUTGOING_RTMP_PACKETS, RTMPServer, RTMPSession, RTMPTSession, RTMPT_SEND_REQUEST_BUFFER_SIZE, SESSION_STATE_HANDSHAKE_DONE, SESSION_STATE_HANDSHAKE_ONGOING, SESSION_STATE_NEW, Sequent, TIMESTAMP_ROUNDOFF, aac, api, avstreams, clientMaxId, codec_utils, config, convertPTSToMilliseconds, counter, createAMF0CommandMessage, createAMF0CommandMessageParams, createAMF0Data, createAMF0DataMessage, createAMF0DataMessageParams, createAMF0ECMAArray, createAMF0Object, createAMF0StrictArray, createAudioMessage, createMessageHeader, createRTMPAggregateMessage, createRTMPMessage, createRTMPType1Message, createVideoMessage, crypto, flushRTMPMessages, flv, generateClientID, generateNewClientID, generateNewSessionID, generateSessionID, h264, logger, net, parseAMF0CommandMessage, parseAMF0Data, parseAMF0DataMessage, parseAMF0ECMAArray, parseAMF0StrictArray, parseAcknowledgementMessage, parseIEEE754Double, parseUserControlMessage, queueAudioMessage, queueRTMPMessages, queueVideoMessage, queuedRTMPMessages, rtmp_handshake, rtmptSessions, rtmptSessionsCount, sessions, sessionsCount, url,
    __slice = [].slice;
  ...
  generateClientID = function() {
    var clientID, i, numPossible, possible, _i;
  ...
  queueRTMPMessages = function(stream, messages, params) {
    ...
    if (queuedRTMPMessages[stream.id] != null) {
      (_ref = queuedRTMPMessages[stream.id]).push.apply(_ref, messages);
    } else {
      queuedRTMPMessages[stream.id] = __slice.call(messages);
    }
  ...
  parseIEEE754Double = function(buf) {
    var bitIndex, bitValue, byteIndex, exponent, fraction, i, sign, value, _i;
    sign = buf[0] >> 7;
  ...
  parseAMF0ECMAArray = function(buf) {
    var bufLen, name, nameLen, obj, readLen, result;
    obj = {};
    bufLen = buf.length;
    readLen = 0;
  ...
  parseAMF0Data = function(buf) {
    ...
    if (type === 0x00) {
      ...
      result = parseAMF0ECMAArray(buf.slice(i + 4));
      return {
        type: 'array',
        value: result.value,
        readLen: i + 4 + result.readLen
      };
    } else if (type === 0x09) {
      return {
  ...
  createAMF0Data = function(data) {
    var buf, count, strBytes, strLen, type;
    type = typeof data;
    buf = null;
    if (type === 'number') {
      buf = new Buffer(9);
  ...
  createAMF0StrictArray = function(arr, buf) {
    var arrLen, bufs, i, totalLength, value, valueBytes, _i, _len;
    if (buf == null) {
      buf = null;
    }
    bufs = [];
  ...
  createAMF0ECMAArray = function(obj, buf) {
  ...
  flushRTMPMessages = function(stream, params) {
    var allSessions, buf, bufs, clientID, i, msgs, rtmpMessage, rtmpMessagesToSend, session, _i, _j, _k, _l, _len, _len1, _len2, _len3;
    if (stream == null) {
    ...
    for (_i = 0, _len = allSessions.length; _i < _len; _i++) {
      session = allSessions[_i];
      ...
      if (session.isWaitingForKeyFrame) {
        if (config.rtmpWaitForKeyFrame) {
          if (stream.isVideoStarted) {
            for (i = _j = 0, _len1 = rtmpMessagesToSend.length; _j < _len1; i = ++_j) {
      ...
      if (session.isPlaying) {
        for (_k = 0, _len2 = msgs.length; _k < _len2; _k++) {
          rtmpMessage = msgs[_k];
          rtmpMessage.timestamp = session.getScaledTimestamp(rtmpMessage.originalTimestamp) % TIMESTAMP_ROUNDOFF;
        }
        if (((params != null ? params.hasControlMessage : void 0) !== true) && (msgs.length > 1)) {
          ...
          bufs = [];
          for (_l = 0, _len3 = msgs.length; _l < _len3; _l++) {
            rtmpMessage = msgs[_l];
  ...
  createRTMPAggregateMessage = function(rtmpMessages, chunkSize) {
    var aggregateBody, aggregateTimestamp, bufs, header, len, rtmpMessage, totalLength, _i, _len;
    bufs = [];
    totalLength = 0;
    aggregateTimestamp = null;
    for (_i = 0, _len = rtmpMessages.length; _i < _len; _i++) {
      rtmpMessage = rtmpMessages[_i];
  ...
  createAMF0DataMessageParams = function(params) {
    var amf0Bytes, len, obj, _i, _len, _ref;
    len = 0;
    _ref = params.objects;
  ...
  createAMF0CommandMessageParams = function(params) {
    var amf0Bytes, commandBuf, len, obj, transactionIDBuf, _i, _len, _ref;
    ...
    amf0Bytes = Buffer.concat([commandBuf, transactionIDBuf].concat(__slice.call(params.objects)), len);
    return {
      chunkStreamID: params.chunkStreamID,
      timestamp: params.timestamp,
      messageTypeID: 0x14,
  ...
  RTMPSession = (function() {
    ...
    RTMPSession.prototype.parseAudioMessage = function(buf) {
      ...
      if (stream == null) {
        throw new Error("[rtmp] Unknown stream: " + this.streamId);
      }
    ...
    RTMPSession.prototype.scheduleTimeout = function() {
      ...
      return this.timeoutTimer = setTimeout((function(_this) {
        return function() {
          ...
          logger.info("[rtmp:" + _this.clientid + "] session timeout");
    ...
    RTMPSession.prototype.removeListener = function(event, listener) {
      var actualIndex, i, listeners, removedCount, _i, _len, _listener, _ref;
      ...
      for (i = _i = 0, _len = listeners.length; _i < _len; i = ++_i) {
        ...
        if (_listener === listener) {
          ...
          [].splice.apply(listeners, [actualIndex, actualIndex - actualIndex + 1].concat(_ref = [])), _ref;
          removedCount++;
        }
      }
    };
    ...
    RTMPSession.prototype.respondConnect = function(commandMessage, callback) {
      var app, connectResult, onBWDone, setPeerBandwidth, streamBegin0, windowAck, _ref;
      app = commandMessage.objects[0].value.app;
      app = app.replace(/\/$/, '');
      if (app !== config.rtmpApplicationName) {
      ...
      connectResult = createAMF0CommandMessage({
        ...
        objects: [
          createAMF0Object({
            ...
            objectEncoding: (_ref = this.objectEncoding) != null ? _ref : 0
          })
        ]
      });
      ...
      return rtmp_handshake.generateS0S1S2(c0c1, (function(_this) {
        return function(err, s0s1s2, keys) {
          ...
          if (type === 6) {
            ...
            logger.info("[rtmp:" + _this.clientid + "] enabled encryption");
          }
    ...
    RTMPSession.prototype.respondReleaseStream = function(requestCommand, callback) {
      var streamName, _ref, _result;
      streamName = (_ref = requestCommand.objects[1]) != null ? _ref.value : void 0;
    ...
    RTMPSession.prototype.receiveSetDataFrame = function(requestData) {
      if (requestData.objects[1].value === 'onMetaData') {
        return logger.debug("[rtmp:receive] @setDataFrame onMetaData");
      } else {
    ...
    RTMPSession.prototype.respondFCUnpublish = function(requestCommand, callback) {
      var streamName, unpublishSuccess, _ref, _result;
      streamName = (_ref = requestCommand.objects[1]) != null ? _ref.value : void 0;
    ...
    RTMPSession.prototype.respondPublish = function(requestCommand, callback) {
      var kv, match, pair, pairs, params, publishStart, publishingName, publishingType, stream, streamName, urlInfo, _i, _len, _ref, _ref1;
      this.receiveTimestamp = null;
      publishingName = (_ref = requestCommand.objects[1]) != null ? _ref.value : void 0;
      ...
      publishingType = (_ref1 = requestCommand.objects[2]) != null ? _ref1.value : void 0;
    ...
    RTMPSession.prototype.respondPlay = function(commandMessage, callback) {
      var codecConfigs, metadata, onMetaData, playReset, playStart, rtmpSampleAccess, setChunkSize, stream, streamBegin1;
      ...
      if (this.streamId != null) {
        ...
        if (stream != null) {
          ...
          logger.error("[rtmp] error: respondPlay: no such stream: " + this.streamId);
        }
      } else {
      ...
      codecConfigs = this.getCodecConfigs();
      callback(null, this.concatenate([setChunkSize, streamBegin1, playReset, playStart, rtmpSampleAccess, onMetaData, codecConfigs]));
      this.isWaitingForKeyFrame = true;
      if (config.rtmpWaitForKeyFrame) {
        return logger.info("[rtmp:" + this.clientid + "] waiting for keyframe");
      }
    };

    RTMPSession.prototype.getCodecConfigs = function() {
      ...
      stream = avstreams.get(this.streamId);
      if (stream == null) {
        logger.error("[rtmp] error: getCodecConfigs: no such stream: " + this.streamId);
        return new Buffer([]);
      }
      if (stream.isVideoStarted) {
        ...
        buf = new Buffer([(1 << 4) | config.flv.videocodecid, 0x00, 0x00, 0x00, 0x00, 0x01].concat(__slice.call(stream.spsNALUnit.slice(1, 4)), [0xff], [0xe1], [(spsLen >> 8) & 0xff], [spsLen & 0xff], __slice.call(stream.spsNALUnit), [0x01], [(ppsLen >> 8) & 0xff], [ppsLen & 0xff], __slice.call(stream.ppsNALUnit)));
    ...
    RTMPSession.prototype.pauseOrUnpauseStream = function(commandMessage, callback) {
      var doPause, _ref;
      doPause = ((_ref = commandMessage.objects[1]) != null ? _ref.value : void 0) === true;
      if (doPause) {
        this.isPlaying = false;
    ...
    RTMPSession.prototype.handleAMFCommandMessage = function(commandMessage, callback) {
      ...
      switch (commandMessage.command) {
        case 'connect':
          this.objectEncoding = (_ref = commandMessage.objects[0]) != null ? (_ref1 = _ref.value) != null ? _ref1.objectEncoding : void 0 : void 0;
          return this.respondConnect(commandMessage, callback);
        case 'createStream':
          return this.respondCreateStream(commandMessage, callback);
        case 'play':
          this.streamId = (_ref2 = commandMessage.objects[1]) != null ? _ref2.value : void 0;
          logger.info("[rtmp:" + this.clientid + "] requested stream " + this.streamId);
          return this.respondPlay(commandMessage, callback);
        case 'closeStream':
        ...
        case 'pause':
          ...
          return this.pauseOrUnpauseStream(commandMessage, callback);
        case 'releaseStream':
    ...
    RTMPSession.prototype.handleData = function(buf, callback) {
      ...
      if (this.state !== SESSION_STATE_HANDSHAKE_DONE) {
        ...
        consumeNextRTMPMessage = (function(_this) {
          return function() {
            var acknowledgementMessage, audioData, bufferLength, commandMessage, dataMessage, dts, newChunkSize, parseResult, pts, rtmpMessage, stream, streamID, timestamp, userControlMessage, videoData, _i, _len, _ref, _results;
            if (buf == null) {
              onConsumeAllPackets();
              return;
            }
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              ...
              switch (rtmpMessage.messageTypeID) {
                ...
                case 8:
                  ...
                  if (audioData.adtsFrame != null) {
                    if (!_this.isFirstAudioReceived) {
                      _this.emit('audio_start', _this.streamId);
                      _this.isFirstAudioReceived = true;
                    }
                    pts = dts = flv.convertMsToPTS(rtmpMessage.timestamp);
                    _this.emit('audio_data', _this.streamId, pts, dts, audioData.adtsFrame);
                  }
                  _results.push(seq.done());
                  break;
                case 9:
                  ...
                  if (videoData.nalUnitGlob != null) {
                    if (!_this.isFirstVideoReceived) {
                      _this.emit('video_start', _this.streamId);
                    ...
                    _this.emit('video_data', _this.streamId, pts, dts, videoData.nalUnitGlob);
                  }
                  if (videoData.isEOS) {
                    logger.info("[rtmp:" + _this.clientid + "] received EOS for stream: " + _this.streamId);
                    stream = avstreams.get(_this.streamId);
                    if (stream == null) {
                      logger.error("[rtmp:" + _this.clientid + "] error: unknown stream: " + _this.streamId);
                    }
                    stream.emit('end');
                  }
                  _results.push(seq.done());
                ...
                case 15:
                  ...
                  _results.push(_this.handleAMFDataMessage(dataMessage, function(err, output) {
                    if (err != null) {
                      logger.error("[rtmp:receive] packet error: " + err);
                    }
                    if (output != null) {
                ...
                case 17:
                  ...
                  _results.push(_this.handleAMFCommandMessage(commandMessage, function(err, output) {
                    if (err != null) {
                      logger.error("[rtmp:receive] packet error: " + err);
                    }
                    if (output != null) {
  ...
  RTMPServer = (function() {
    function RTMPServer(opts) {
      var _ref;
      this.eventListeners = {};
      this.port = (_ref = opts != null ? opts.rtmpServerPort : void 0) != null ? _ref : 1935;
      this.server = net.createServer((function(_this) {
        return function(c) {
          ...
          sess.on('video_start', function() {
            ...
            return _this.emit.apply(_this, ['video_start'].concat(__slice.call(args)));
          });
          sess.on('audio_start', function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            return _this.emit.apply(_this, ['audio_start'].concat(__slice.call(args)));
          });
          sess.on('video_data', function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            return _this.emit.apply(_this, ['video_data'].concat(__slice.call(args)));
          });
          sess.on('audio_data', function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            return _this.emit.apply(_this, ['audio_data'].concat(__slice.call(args)));
          });
          c.on('close', function() {
    ...
    RTMPServer.prototype.start = function(opts, callback) {
      var serverPort, _ref;
      serverPort = (_ref = opts != null ? opts.port : void 0) != null ? _ref : this.port;
    ...
    RTMPServer.prototype.teardownRTMPTClient = function(socket) {
      ...
      if (socket.rtmptClientID != null) {
        ...
        if (tsession != null) {
          if ((_ref = tsession.rtmpSession) != null) {
            _ref.teardown();
          }
    ...
    RTMPServer.prototype.sendVideoPacket = function(stream, nalUnits, pts, dts) {
      var buf, compositionTimeMs, firstByte, hasKeyFrame, message, nalUnit, nalUnitType, payloadLen, timestamp, _i, _len;
      ...
      compositionTimeMs = Math.round((pts - dts) / 90);
    ...
    RTMPServer.prototype.sendEOS = function(stream) {
      var lastTimestamp, playComplete, playStop, streamEOF1, _ref;
      lastTimestamp = (_ref = stream.rtmpLastTimestamp) != null ? _ref : 0;
    ...
    RTMPServer.prototype.handleRTMPTRequest = function(req, callback) {
      var client, command, index, info, match, response, session, _results;
          ...
          if (session != null) {
            ...
            if (session.requestBuffer != null) {
              ...
              while ((session.requestBuffer.reqs.length > 0) && (session.requestBuffer.reqs[0].index === session.requestBuffer.nextIndex)) {
                ...
                _results.push(session.requestBuffer.nextIndex = info.index + 1);
              }
              return _results;
            } else {
  ...
  RTMPTSession = (function() {
    ...
    RTMPTSession.prototype.respondIdle = function(req, callback) {
      var allBytes, bufs, resp, totalLength, _i, _len, _ref;
      this.scheduleTimeout();
      bufs = [new Buffer([this.pollingDelay])];
      totalLength = 1;
      _ref = this.pendingResponses;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        resp = _ref[_i];
        bufs.push(resp);
